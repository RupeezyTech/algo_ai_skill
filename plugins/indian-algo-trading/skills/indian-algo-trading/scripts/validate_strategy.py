...
class StrategyValidator(ast.NodeVisitor):
    ...
    def __init__(self, filename):
        ...
        self.try_body_ranges = []  # (start_lineno, end_lineno) of try-body statements only
        self.place_order_calls = []  # linenos of every *.place_order(...) call
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if 'place_order' in func_name:
                self.place_order_calls.append(node.lineno)
    ...
    def visit_Try(self, node):
        # Record the source range of the try BODY only (not except/else/finally --
        # a place_order in an except handler is not protected by the try).
        for stmt in node.body:
            start = stmt.lineno
            end = getattr(stmt, 'end_lineno', stmt.lineno)
            self.try_body_ranges.append((start, end))
        self.generic_visit(node)

    def get_unwrapped_place_orders(self):
        """Return linenos of place_order calls that fall outside every try-body range."""
        return [
            lineno for lineno in self.place_order_calls
            if not any(start <= lineno <= end for start, end in self.try_body_ranges)
        ]
    ...
    def check_string_content(self, tree):
        ...
        if 'nseindia.com' in source or 'requests.get' in source and 'nseindia' in source.lower():
            self.has_nseindia_scrape = True
...
def validate_file(filepath):
    ...
    unwrapped_place_orders = validator.get_unwrapped_place_orders()
    ...
    checks = {
        ...
        'ERROR_HANDLING': ('WARN', [f'Line {lineno}: Missing try/except around place_order' for lineno in unwrapped_place_orders]),
